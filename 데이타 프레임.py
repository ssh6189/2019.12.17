Pandas의 2차원 자료구조 객체(Matrix) - DataFrame(컬럼, 로우, 데이터, 인덱스)
...
df2 = df[:]  #원본 객체 변경이 아닌 새로운 DataFrame을 복ㅈ한다.
result = df2.drop('삼식') #행 인덱스(label)로 삭제한 새로운 데이터 프레임 객체 생성 반환한다.
print(df2)
print(result)

df.drop('삼식', inplace=True)
print(df2)
...
df.drop('삼식', inplace=True) #행 삭제한 새로운 뎅터 프레임 객체 반환대신 자기자신의 행 삭제
print(df2)
...
#행 선택
student1 = df.loc['철수'] #label을 사용해서 행을 선택
print(student1)
student2 = df.iloc[0] #index를 사용해서 행을 선택
print(student2)

#행 선택
student3 = df.loc['철수', '삼식'] #행 인덱스(lavel) 범위 지정, 행을 선택
print(student1)
student4 = df.iloc[[0, 1]] #index를 사용해서 2개 이상 행을 선택
print(student2)
